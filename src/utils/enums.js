export const sessionData = {
  appId: "EW1LP8KAGZ-100",
  redirectURL: "https://trade.fyers.in/api-login/redirect-uri/index.html",
  secretKey: "79TLHP67JH",
  accessToken:
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..pPj4VFDcB11GnsNqTGNS3NIFvEOsE81t7IaVR_GqW9A",
};

const host = "http://13.233.55.6";
// const host = "http://localhost";

export const apiEndpoints = {
  getAccessToken: `${host}:3000/api/v1/token/getToken`,
  createAccessToken: `${host}:3000/api/v1/token/createToken`,
  getDelayedData: `${host}:3000/api/v1/options/get-options-data`,
  deleteAllData: `${host}:3000/api/v1/options`,
};
